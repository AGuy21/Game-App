...
import { CountContext, UserContext } from '../../app/_layout';
...
const LeaderBoardData = () => {
    ...
    const userCount = countContext?.count
    ...
    // all data states
    const [lbData, setLbData] = useState<{ name: string; count: number; character: number }[] | any>(); // to use data as global
    ...
    const getData = async () => { // gets the users data
        ...
        const getCollection = await getDocs(collection(FIREBASE_DB, "count"));
            getCollection.forEach((doc: any) => {
                userData[doc.data().Username] = [doc.data().count, doc.data().Character] ; // adds the data from the doc into the object to be user
                console.log(userData); // for debugging
            });
            
    };

    const saveCount = async () => { // saves all data when called 
        const countSet = await updateDoc( docRef , { count: userCount})
        console.log('Saved Count For Leaderboard')
    }
    
    useEffect(() => { // to run all data again if the user refreshes
        console.log("Refreshing Data..."); // so dev knows if data is being refreshed
        setLoading(true);
        saveCount();
        ...
        // IF the data is retrieved it will then map that data for use
            ...
            let Timeout = setTimeout(() => { // sets a timeout so the data can have time to be set
                const dataArray = Object.entries(userData).map(([name, data]) => ({
                    name,
                    count: data[0], // data one is count so we go to array index 1 to get count data
                    character: data[1],
                })); 
                console.log("len" + dataArray.length)
            // AFTER data is converted
                // SORTING DATA FOR LEADERBOARD BELOW
                console.log('Before Sorted ' + dataArray) // for debugging
                const sortedData = dataArray.sort((a, b) => b.count - a.count); // sorts the count for the leaderboard from highest to lowest
                console.log('After Sorted ' + lbData) // for debugging

                setLbData(sortedData); // to set the data to be used in the leaderboard as the sorted data

                console.log("Data Converted!");
                console.log(lbData); // for debugging

                // FINDING RICHEST FACTION BELOW
                ...
                for (let i = 0; i < sortedData.length; i++) {
                    if (sortedData[i].character === 1) {
                        knight += sortedData[i].count
                    } else {
                        assassin += sortedData[i].count
                    }
                }
                ...
                console.log("Strongest Fac: " + strongestFac) // for debugging
                setLoading(false)
            }, 1000)
        }
        ...
        {loading ? (
            ...
            <View style={{ alignItems: 'center'}}>
                <View style={styles.headerContainer}>
                    ...
                    <Text style={styles.statText}> {userCount} </Text>
                </View>
                <View style={styles.compareContainer}>
                    ...
                    <View style={{ flexDirection: 'row', gap: wp(1), justifyContent: 'center'}}>
                        <Text style={styles.compareText}> Knight's Dark Matter:  </Text>
                    ...
                    <View style={{ flexDirection: 'row', gap: wp(1), justifyContent: 'center'}}>
                        <Text style={styles.compareText}> Assassin's Dark Matter:   </Text>
                ...
                <FlatList 
                    ...
                    data={lbData?.slice(0, 5)} // to only show the top 5 users
